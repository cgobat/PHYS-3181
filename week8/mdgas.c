...
//printf("argc: %d dt: %f nstep: %d\n", argc, dt, nstep);
  ...
  printf("%e ", 0.0);
  for(int i=0; i<2*N*DIM; ++i) printf("%e ", x[i]);
  printf("\n");
  ...
  for(int i=0; i<nstep; ++i)
  {
    ...
    for(int k=0; k<N; ++k)
    {
      if(xf[4*k] < 0)   { xf[4*k+0] *= -1; xf[4*k+2] *= -1; }
      if(xf[4*k] > L)   { xf[4*k+0] = 2*L - xf[4*k]; xf[4*k+2] *= -1; }
      if(xf[4*k+1] < 0) { xf[4*k+1] *= -1; xf[4*k+3] *= -1; }
      if(xf[4*k+1] > L) { xf[4*k+1] = 2*L - xf[4*k+1]; xf[4*k+3] *= -1; }
    }

    printf("%e ", i*dt+dt);
    for(int j=0; j<2*N*DIM; ++j) printf("%e ", xf[j]);
    printf("\n");
    for(int j=0; j<2*N*DIM; ++j) x[j] = xf[j];
  }
